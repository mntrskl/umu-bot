let values = process.env.platform != 'Heroku' ? require('./config.json') : process.env;
const config = {
	// Bot Owner, level 10 by default. A User ID. Should never be anything else than the bot owner's ID.
	ownerID: values.ID_OWNER,
		...
		// This is the server owner.
		{
			...
			check: (message) =>
				message.channel.type === 'text'
					? message.guild.owner.user.id === message.author.id ? true : false
					: false
		},
